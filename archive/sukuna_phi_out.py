...
for name_num in range(9):
  ...
  for set_num in (10,20,30):
    for i in range(10):#calculate the avelage value of estimated accuracy for datum in this loop
      ...
      check_i = 3
      phi_test[(set_num/10-1),i,name_num] = np.average(corr[check_i,~np.isnan(corr[check_i])])
      #phi_test[(set_num/10-1),i,name_num] = corr[~np.isnan(corr)]/((float)(set_num))
      ...
      phi_train[(set_num/10-1),i,name_num] = np.average(corr[check_i,~np.isnan(corr[check_i])])
      #phi_train[(set_num/10-1),i,name_num] = corr[~np.isnan(corr)]/((float)(set_num))
