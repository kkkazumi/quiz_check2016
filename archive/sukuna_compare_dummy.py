...
for name_num in range(hito_num):
  ...
  for set_num in set_list:
    ...
    nn_corr = np.loadtxt(dir_name + "/dummy_corr-" + str(set_num) + ".csv", delimiter=",")
    nn_p = np.loadtxt(dir_name + "/dummy_p-" + str(set_num) + ".csv")
    ...
    print nn_ave[set_num/5-2,name_num]
    raw_input()
...
data['way'].str.replace('88.0','dummy')
...
g2=plt.plot(set_list,nave,color='b',label='dummy',marker='o',linestyle='dashed')
...
g2=plt.plot(set_list,ncorr,color='b',marker='o',label='dummy',linestyle='dashed')
plt.xlabel('number of training data',fontsize=15)
