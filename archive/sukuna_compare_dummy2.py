...
nn_all = np.zeros((len(set_list),hito_num,test_len*1))
phi_all = np.zeros((len(set_list),hito_num,test_len*1))
...
for name_num in range(hito_num):
  ...
  for set_num in set_list:
    ...
    nn_corr1 = np.loadtxt(dir_name + "/dummy_corr-" + str(set_num) + ".csv", delimiter=",")
    #nn_corr2 = np.loadtxt(dir_name2 + "/nn_corr-" + str(set_num) + ".csv", delimiter=",")
    nn_p1 = np.loadtxt(dir_name + "/dummy_p-" + str(set_num) + ".csv")
    ...
    nn_num[set_num/5-1,name_num] = np.count_nonzero((nn_corr)>0.6)
    phi_num[set_num/5-1,name_num] = np.count_nonzero((phi_corr)>0.6)
...
df_phi_in = pd.DataFrame(data=index,columns=["the number of supervisor data"],dtype='int')
df_nn_way = pd.DataFrame({'way':['same weight','same weight','same weight','same weight','same weight','same weight']},columns=["way"],dtype='object',index=[0,1,2,3,4,5])
...
for i in range(len(set_list)):
  ...
  #Jphi_num[set_num/5-1,name_num] = np.count_nonzero((phi_corr)>0.2)
  #val2 = phi_num[i,:]
  pcorr[i] = np.sum(phi_num[i,:])
  #print "npcount",np.count_nonzero(val2[~np.isnan(val2)])
  #pcorr[i] = np.average(val2[~np.isnan(val2)])

  ncorr[i]= np.sum(nn_num[i,:])
