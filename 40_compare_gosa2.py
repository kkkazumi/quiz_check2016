...
set_list = (25,26,27,28,29)#, 30,35,40)
...
for name_num in range(hito_num):
  ...
  for set_num in set_list:
    ...
    phi_corr[np.isnan(nn_corr)] = 0
    phi_corr[np.isnan(phi_corr)] = 0
    nn_corr[np.isinf(phi_corr)] = 0
    nn_corr[np.isinf(nn_corr)] = 0
    nn_corr[np.isnan(phi_corr)] = 0
    ...
    nn_ave[set_num-25,name_num]= np.average(abs(nn_corr))
    phi_ave[set_num-25,name_num]= np.average(abs(phi_corr))
...
#sns_plot = sns.violinplot(x='the number of supervisor data',y='difference between estimated mood and self-assessed mood',hue='way',data=data_new,capsize=.2)
