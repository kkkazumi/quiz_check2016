...
NUM_MAX = 40
TEST_NUM = 10 #the number of test data is 10
...
for i_name in username:
  ...
  #x_train = np.vstack((x_train_b,x_train_a))
  #y_train= np.vstack((ydata_a,ydata_a))

  test_index = np.zeros(10,dtype='int')

  for set_num in (10,20,30):
    test_index0 = np.random.choice(range(0, NUM_MAX-10),10,replace=False) #the first index to start 10 test data sequence.
