...
for selected_num in range(9):
  name_num = selected_num
#for name_num in range(hito_num):
  for t in range(hito_num):
    ...
    for set_num in set_list: 
      ...
      nn_all[set_num-25,name_num,:]= nn_corr
      phi_all[set_num-25,name_num,:]= phi_corr
      print('nn',nn_corr[nn_corr>0].shape)
      print('phi',phi_corr[phi_corr>0].shape)

      if flag == "abs":
        #abs
        nn_ave[set_num-25,name_num]= np.average((nn_corr))
        phi_ave[set_num-25,name_num]= np.average((phi_corr))
        str_label = 'the average of the absolute value of the correlation \n between estimated mood and self-assessed mood'

      elif flag == "plus":
        #plus
        nn_ave[set_num-25,name_num]= np.average(nn_corr[nn_corr>0])
        phi_ave[set_num-25,name_num]= np.average(phi_corr[phi_corr>0])
        str_label = 'the average of the positive correlation \n between estimated mood and self-assessed mood'

      elif flag == "num":
        #num
        nn_ave[set_num-25,name_num]= nn_corr[nn_corr>0].shape[0]
        phi_ave[set_num-25,name_num]= phi_corr[phi_corr>0].shape[0]
  ...
  index[:,0] = np.linspace(25,29,len(set_list),dtype='int')
...
#data_new = data.rename(columns={'value':'correlation between estimated mood and self-assessed mood'})
  ...
  #sns_plot = sns.violinplot(x='the number of supervisor data',y=str_label,hue='way',data=data_new,capsize=.2)
...
#plt.ylim([0,0.3])
  ...
  plt.title(str(selected_num))
  plt.show()
  #plt.savefig('5-40diff-'+flag+"-"+str(selected_num)+'.png')
