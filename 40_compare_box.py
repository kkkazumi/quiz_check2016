...
for name_num in range(hito_num):
  ...
  print np.average(nn_corr[nn_corr>0])
  print np.average(phi_corr[phi_corr>0])
...
index[:,0] = np.linspace(0,24,int(test_len),dtype='int')

df_phi_in = pd.DataFrame(data=index,columns=["test no"],dtype='int')
df_phi_way = pd.DataFrame({graph_label:"proposed method"},columns=[graph_label],dtype='object',index=np.linspace(0,24,int(test_len),dtype='int'))
...
df_nn_way = pd.DataFrame({graph_label:"neural network"},columns=[graph_label],dtype='object',index=np.linspace(0,24,int(test_len),dtype='int'))
...
sns_plot = sns.violinplot(x='user id',y='correlation between self-assessed mood and estimated mood',hue=graph_label,data=data_new)
